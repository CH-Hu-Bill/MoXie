...
| **管理后台** (`admin.php`) | 班级删除（级联清理）、重置班级口令、APP 版本发布（含渠道/日志）、**APK 安装包上传**（只保留最新一版，写入 `app_versions.json` 的 `apk` 字段）、**全服公告管理**（内容/颜色/班级/平台/时间/可关闭，超级霸屏限 60 字、顶部横幅限 200 字，前端实时计数）。页面已加 no-store 防缓存 + 分区导航（班级/APP/公告/大屏） |
