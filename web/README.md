...
| **全服公告** | 管理员发布的公告按班级 + 平台 + 时间段投送；Web 端有公告时顶部状态栏左侧显示"班级 \| 当前页面"、空白处为无缝跑马灯（使用公告自定义颜色，可关闭，存 localStorage），关闭后恢复居中页面标题；APP 端顶部横幅位于 AppBar 下方（文本未超出静态显示、超出时无缝滚动且两侧渐变蒙板，可关闭）。后台时间选择已预填服务器当前时间（默认立即生效），并醒目显示服务器时间 |
