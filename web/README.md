...
| **单词库** (`words.php`) | 单词 CRUD、TTS 发音、AI 批量导入（粘贴单词列表 → DeepSeek 自动补全释义/词性 → 预览确认）、CSV 导入 / 导出、**跟读**（全部/已选单词，可随机乱序——只打乱播放顺序不改单词库位置） |
| **默写任务** (`task.php`) | 三种模式：**看词**(show) / **默写**(hide) / **听写**(dict)；听写状态机（音量/间隔/朗读次数可调，支持暂停）；短按完成、长按取消；**跟读**（任务单词，乱序可选） |
| **默写记录** (`history.php`) | 历史任务列表（按日期倒序）、单词详情、一键重新创建任务 |
| **周末大礼包** | 周末从本周已默写单词中随机抽 20 个组成加练任务，周一随机决定本周是否开启 |
| **班级史记** (`history_book.php`) | Vlog 风格日记，Quill 富文本编辑器 + 月历导航；仅今日可编辑（带时钟容差，详见下文）；个人列传（需授权）；支持 PDF / HTML / 长图导出；**手写板用 Pointer Events 统一鼠标/触摸/笔**（`touch-action:none` 防触屏手势被浏览器接管滚动页面，高分屏按 devicePixelRatio 渲染不发糊，弹窗期间锁定 body 滚动）；**编辑器支持拖拽图片直接上传插入**（配合原有 Ctrl+V 粘贴，绕开 Windows 触屏设备文件选择对话框卡死问题） |
...
| **下载 APP** (`download_app.php`) | 公开页面：展示最新版本号/上传时间/大小/更新说明 + 下载按钮 + 历史版本记录；APK 存 `apk/listenwrite-release.apk`（后台上传，只保留最新一版） |

> **APP 版本发布与 APK 上架流程**（管理后台 `/admin.php`）：
> 1. 「APP 版本发布」：填版本号（如 `1.0.11`）+ 更新说明 → 写入 `data/app_versions.json`（`latest` / `history`），APP 端 `check_version` 据此弹更新提示。
> 2. 「上传安装包」：只选 `.apk` 文件（一个按钮）→ **自动关联「发布新版本」中的最新版本号与更新日志**，无需重复填写；只保留最新一版（覆盖旧包）。
> 3. 用户端：主页「下载 APP」→ `download_app.php` 展示版本号/大小/上传时间/更新说明 + 下载按钮；或 APP 内升级弹窗提示。
...
| `gallery.php` | 图集：上传（图片/GIF/MP4）/ 编辑描述 / 删除 / 画廊，Lightbox 播放视频带声音开关；**上传区支持拖拽与 Ctrl+V 粘贴**（绕开 Windows 触屏文件选择对话框卡死），Lightbox 视频加载中显示**缓冲进度百分比**（`preload=auto` + progress 事件，避免低带宽下"点开没反应"的错觉）；界面图标全部为 SVG 线条风（不用 emoji） | `?id={classId}` |
...
- **单词**：取**最近一次任务**（按 `created_at` 最新、跳过已取消）的单词（上限 20），不再固定展示"今天"的任务；顶部标题显示任务日期（含星期）+ 标签。**字号自适应**：按内容区宽高 + 单词数（75 分位长度）动态算列数与字号（16~64px），个别超长单词单独缩小该卡片并滚动展示，保证后排可读
...
- 60s 轮询一次（公告/单词/图集列表），图集 30s 纯客户端切换 + 预加载
