...
| **全服公告** | 管理员发布的公告按班级 + 平台 + 时间段投送；Web 端在顶部状态栏显示（左侧始终显示"班级 + 当前页面"，右侧空白处为跑马灯，使用公告自定义颜色，可关闭，存 localStorage）；APP 端顶部横幅显示（可关闭，存 SharedPreferences）。后台时间选择已预填服务器当前时间（默认立即生效），并醒目显示服务器时间 |
