...
| 画廊 | 网格浏览+上传（图片/GIF/MP4 视频，本地校验格式/大小，视频≤30s/15MB 不压缩保留原始编码）+大图查看+图片缓存；**MP4 视频卡片显示后端 ffmpeg 生成的首帧图（cached_network_image 缓存秒开）+ 手绘风「视频」贴纸（白纸卡片+抖动圆角+硬阴影），点进去才加载真视频**（首次网络流式播放并后台缓存整文件，二次起直接读缓存）；GIF 动图缩略图进入视口才播放、离开释放；大图可缩放/播放/声音开关；**大图查看时描述过长在固定区域自动滚动（可拖动打断、2s 后恢复）**；支持修改图片描述；下拉刷新+30s 自动刷新 |
